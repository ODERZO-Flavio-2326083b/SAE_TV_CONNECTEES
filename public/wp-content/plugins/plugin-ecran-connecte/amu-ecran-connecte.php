 ...
 * Description:       Plugin écrans connectés de l'AMU, ce plugin permet de générer
 *                    des fichiers ICS. Ces fichiers sont ensuite lus pour pouvoir afficher l'emploi du temps de la personne connectée. Ce plugin permet aussi d'afficher la météo, des informations, des alertes. Tant en ayant une gestion des utilisateurs et des informations.
 * Version:           1.2.9
 ...
 * @return void
 */
function downloadSchedule($users) : void
{
    $controllerAde = new CodeAdeController();
...
function move_fileICS_schedule() : void
{
    ...
    if ($myFiles = scandir($_SERVER['DOCUMENT_ROOT'] . TV_ICSFILE_PATH . 'file2')) {
        foreach ($myFiles as $myFile) {
            if (is_file(
                $_SERVER['DOCUMENT_ROOT']
                . TV_ICSFILE_PATH . 'file2/' . $myFile
            )
            ) {
                copy(
                    $_SERVER['DOCUMENT_ROOT']
                    . TV_ICSFILE_PATH . 'file2/'
    ...
    if ($myFiles = scandir($_SERVER['DOCUMENT_ROOT'] . TV_ICSFILE_PATH . 'file1')) {
        foreach ($myFiles as $myFile) {
            if (is_file(
                ...
                copy(
                    $_SERVER['DOCUMENT_ROOT']
                    . TV_ICSFILE_PATH . 'file1/'
                    . $myFile, $_SERVER['DOCUMENT_ROOT']
                    . TV_ICSFILE_PATH . 'file2/' . $myFile
                );
                wp_delete_file(
                    $_SERVER['DOCUMENT_ROOT']
                    . TV_ICSFILE_PATH . 'file1/' . $myFile
                );
            }
        }
    }
    ...
    if ($myFiles = scandir($_SERVER['DOCUMENT_ROOT'] . TV_ICSFILE_PATH . 'file0')) {
        foreach ($myFiles as $myFile) {
            if (is_file(
                $_SERVER['DOCUMENT_ROOT'] . TV_ICSFILE_PATH . 'file0/' . $myFile
            )
            ) {
                copy(
                    $_SERVER['DOCUMENT_ROOT']
...
function displayParticipant()
{
    ...
    return '<p class="info-text"> - '
        .$result1.'</p><p class="info-text"> - '.$result2.'</p>';
}
