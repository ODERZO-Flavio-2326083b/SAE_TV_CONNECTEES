...
class ICal
{
    ...
    public function iCalDateToDateTime($icalDate, $forceTimeZone = false, $forceUtc = false)
    {
        ...
        if ($year <= $this::UNIX_MIN_YEAR) {
            ...
            if ($forceTimeZone) {
                ...
                if ($date[8] === 'Z') {
                    $isUtc    = true;
                    $dateTime = new \DateTime('now', new \DateTimeZone($this::TIME_ZONE_UTC));
        ...
        if ($forceTimeZone && $isUtc) {
            ...
            $dateTime->setTimezone(new \DateTimeZone($this::TIME_ZONE_UTC));
        }
    ...
    public function iCalDateWithTimeZone(array $event, $key, $format = self::DATE_TIME_FORMAT)
    {
        ...
        if ($key === 'DURATION') {
            ...
            $dateTime = new \DateTime($dateArray[1], new \DateTimeZone($this::TIME_ZONE_UTC));
            ...
            foreach (array('DTSTART', 'DTEND', 'RECURRENCE-ID') as $type) {
                if (isset($anEvent[$type])) {
                    ...
                    if (isset($anEvent[$type . '_array'][0]['TZID'])) {
                        $date = sprintf($this::ICAL_DATE_TIME_TEMPLATE, $anEvent[$type . '_array'][0]['TZID']) . $date;
                    }
    ...
    protected function processRecurrences()
    {
        ...
        foreach ($events as $anEvent) {
            if (isset($anEvent['RRULE']) && $anEvent['RRULE'] !== '') {
                ...
                $anEvent['RRULE_array'][2] = $this::RECURRENCE_EVENT;
                ...
                if (isset($rrules['UNTIL'])) {
                    ...
                    if (in_array($frequency, array('MONTHLY', 'YEARLY'))
                        ...
                        $until = max($until, $dtstart->format($this::UNIX_FORMAT));
                    }
                ...
                switch ($frequency) {
                    case 'DAILY':
                        ...
                        while ($recurringTimestamp <= $until) {
                            ...
                            if ($this->useTimeZoneWithRRules) {
                                $recurringTimeZone = \DateTime::createFromFormat($this::UNIX_FORMAT, $dayRecurringTimestamp);
                            ...
                            // Add event
                            $anEvent['DTSTART'] = date($this::DATE_TIME_FORMAT, $dayRecurringTimestamp) . ($isAllDayEvent || ($initialStartTimeZoneName === 'Z') ? 'Z' : '');
                            ...
                            $isExcluded = array_filter($exdates, function ($exdate) use ($anEvent, $dayRecurringOffset) {
                                return $this::isExdateMatch($exdate, $anEvent, $dayRecurringOffset);
                            });

                            if (isset($anEvent['UID'])) {
                                $searchDate = $anEvent['DTSTART'];
                                if (isset($anEvent['DTSTART_array'][0]['TZID'])) {
                                    $searchDate = sprintf($this::ICAL_DATE_TIME_TEMPLATE, $anEvent['DTSTART_array'][0]['TZID']) . $searchDate;
                                }
                    ...
                    case 'WEEKLY':
                        ...
                        while ($weekRecurringTimestamp <= $until) {
                            ...
                            $dayRecurringOffset = 0;
                            if ($this->useTimeZoneWithRRules) {
                                $dayRecurringTimeZone = \DateTime::createFromFormat($this::UNIX_FORMAT, $dayRecurringTimestamp);
                        ...
                        if (isset($rrules['BYMONTHDAY']) && $rrules['BYMONTHDAY'] !== '') {
                                ...
                                foreach ($monthdays as $key => $monthday) {
                                    if ($key === 0) {
                                        ...
                                        $anEvent['DTSTART'] = gmdate(
                                                'Ym' . sprintf('%02d', $monthday) . '\T' . $this::TIME_FORMAT,
                                    ...
                                    if ($this->useTimeZoneWithRRules) {
                                        $recurringTimeZone = \DateTime::createFromFormat($this::UNIX_FORMAT, $monthRecurringTimestamp);
                                        $recurringTimeZone->setTimezone($initialStart->getTimezone());
                                    ...
                                    $isExcluded = array_filter($exdates, function ($exdate) use ($anEvent, $monthRecurringOffset) {
                                        return $this::isExdateMatch($exdate, $anEvent, $monthRecurringOffset);
                                    });
                            ...
                            while ($recurringTimestamp <= $until) {
                                ...
                                $eventStartDesc = "{$this->convertDayOrdinalToPositive($dayNumber, $weekday, $monthRecurringTimestamp)} {$this->weekdays[$weekday]} of "
                                    . date($this::DATE_TIME_FORMAT_PRETTY, $monthRecurringTimestamp);
                                ...
                                do {
                                    ...
                                    if ($eventStartTimestamp > $startTimestamp && $eventStartTimestamp < $until) {
                                        $anEvent['DTSTART'] = date($this::DATE_TIME_FORMAT, $eventStartTimestamp) . ($isAllDayEvent || ($initialStartTimeZoneName === 'Z') ? 'Z' : '');
                                        $anEvent['DTSTART_array'][1] = $anEvent['DTSTART'];
                                    ...
                                    $eventStartTimestamp += $this::SECONDS_IN_A_WEEK;
                    ...
                    case 'YEARLY':
                        ...
                        if (isset($rrules['BYDAY']) && $rrules['BYDAY'] !== '') {
                            while ($recurringTimestamp <= $until) {
                                ...
                                if ($this->useTimeZoneWithRRules) {
                                    $recurringTimeZone = \DateTime::createFromFormat($this::UNIX_FORMAT, $yearRecurringTimestamp);
                                    $recurringTimeZone->setTimezone($initialStart->getTimezone());
                                    ...
                                    do {
                                        if ($eventStartTimestamp > $startTimestamp && $eventStartTimestamp < $until) {
                                            ...
                                            $isExcluded = array_filter($exdates, function ($exdate) use ($anEvent, $yearRecurringOffset) {
                                                return $this::isExdateMatch($exdate, $anEvent, $yearRecurringOffset);
                                            });
                            ...
                            while ($recurringTimestamp <= $until) {
                                ...
                                if (isset($rrules['BYMONTH']) && $rrules['BYMONTH'] !== '') {
                                    ...
                                    array_push($eventStartDescs, $day . gmdate($this::DATE_TIME_FORMAT_PRETTY, $yearRecurringTimestamp));
                                }
    ...
    protected function processDateConversions()
    {
        ...
        foreach ($events as $key => $anEvent) {
            ...
            if ($this->useTimeZoneWithRRules && isset($anEvent['RRULE_array'][2]) && $anEvent['RRULE_array'][2] === $this::RECURRENCE_EVENT) {
    ...
    protected function processEventIcalDateTime(array $event, $index = 3)
    {
        ...
        foreach (array('DTSTART', 'DTEND', 'RECURRENCE-ID') as $type) {
            if (isset($event["{$type}_array"])) {
                ...
                $event["{$type}_array"][$index] = ((is_null($timeZone)) ? '' : sprintf($this::ICAL_DATE_TIME_TEMPLATE, $timeZone)) . $event["{$type}_array"][1];
            }
        }
    ...
    public function calendarTimeZone($ignoreUtc = false)
    {
        ...
        if ($ignoreUtc && strtoupper($timeZone) === $this::TIME_ZONE_UTC) {
            return null;
        }
    ...
    protected function numberOfDays($days, $start, $end)
    {
        ...
        $oneWeek = $this::SECONDS_IN_A_WEEK;
    ...
    protected function convertDayOrdinalToPositive($dayNumber, $weekday, $timestamp)
    {
        ...
        $timestamp = (is_object($timestamp)) ? $timestamp : \DateTime::createFromFormat($this::UNIX_FORMAT, $timestamp);
        $start     = strtotime('first day of ' . $timestamp->format($this::DATE_TIME_FORMAT_PRETTY));
        $end       = strtotime('last day of ' . $timestamp->format($this::DATE_TIME_FORMAT_PRETTY));
            ...
            foreach ($subArray as $key => $value) {
                if ($key === 'TZID') {
                    ...
                    if (substr($icalDate, -1) === 'Z') {
                        $currentTimeZone = $this::TIME_ZONE_UTC;
                    }
