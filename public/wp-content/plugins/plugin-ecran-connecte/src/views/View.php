...
class View
{
    ...
    /**
     * Génère un tableau HTML affichant une liste d'éléments avec des options de tri
     * et de recherche.
     *
     ...
     *
     * @param string $name       Le nom du tableau, utilisé pour les fonctionnalités
     *                           de sélection.
     * @param string $title      Le titre du tableau à afficher en
     *                           haut.
     * @param array  $dataHeader Tableau des en-têtes de colonnes.
     * @param array  $dataList   Liste des données à afficher dans le
     *                           tableau, chaque élément étant un tableau
     *                           représentant une ligne.
     * @param string $idTable    (Optionnel) Identifiant unique pour le tableau
     *                           et les fonctionnalités associées (recherche,
     *                           tri).
     *
    ...
    public function displayAll($name, $title, $dataHeader, $dataList,
        ...
        $table = '
        <h2>' . $title . '</h2>
        <input type="text" id="key' . $idTable . '" name="key" onkeyup="search(\''
            . $idTable . '\')" placeholder="Recherche...">
        <form method="post">
            ...
            . '">
                    <thead>
                        <tr class="text-center">
                            <th width="5%" class="text-center" onclick="sortTable(
                                0, \'' . $idTable . '\')">#</th>
                            <th scope="col" width="5%" class="text-center"><input 
                            type="checkbox" onClick="toggle(this, ' . $name . ')" />
                            </th>';
        $count = 1;
        foreach ($dataHeader as $data) {
            ++$count;
            $table .= '<th scope="col" class="text-center" onclick="sortTable('
                . $count . ', \'' . $idTable . '\')">' . $data . '</th>';
        }
        $table .= '
            </tr>
        </thead>
        ...
        $table .= '
            ...
            <button type="submit" class="btn delete_button_ecran" value="Supprimer" 
            name="delete" onclick="return confirm(
                \' Voulez-vous supprimer le(s) élément(s) sélectionné(s) ?\');">
                Supprimer</button>
        </form>';
        return $table;
    }
    ...
    /**
     * Génère une pagination pour naviguer à travers les pages d'une liste
     * d'éléments.
     *
     * Cette méthode crée un ensemble de liens de pagination qui permettent à
     * l'utilisateur de naviguer entre différentes pages de résultats. Elle inclut
     * des liens vers la page précédente, les numéros de pages environnantes, et la
     * page suivante, tout en gérant les cas où les pages sont nombreuses.
     *
     * @param int      $pageNumber    Le nombre total de pages disponibles.
     * @param int      $currentPage   Le numéro de la page actuelle.
     * @param string   $url           L'URL de base pour générer les liens de
     *                                pagination.
     * @param int|null $numberElement (Optionnel) Le nombre d'éléments par page,
     *                                utilisé pour construire l'URL si nécessaire.
     *
     ...
     * @date    2024-10-15
     */
    public function pageNumber($pageNumber, $currentPage, $url,
        $numberElement = null
    ) : string {
        $pagination = '
        ...
        if ($currentPage > 1) {
            ...
            <li class="page-item">
              <a class="page-link" href="' . $url . '/' . ($currentPage - 1)
                . '/?number=' . $numberElement . '" aria-label="Previous">
                <span aria-hidden="true">&laquo;</span>
              </a>
            </li>
            <li class="page-item"><a class="page-link" href="' . $url . '/1/?number='
                . $numberElement . '">1</a></li>';
        }
        if ($currentPage > 3) {
        ...
        for ($i = $currentPage - 3; $i < $currentPage; ++$i) {
            if ($i > 1) {
                $pagination .= '<li class="page-item"><a class="page-link" href="'
                    . $url . $i . '/?number=' . $numberElement . '">' . $i . '</a>
</li>';
            }
        }
        $pagination .= '
        <li class="page-item active_ecran" aria-current="page">
          <a class="page-link" href="' . $url . $currentPage . '/?number='
            . $numberElement . '">' . $currentPage . '<span class="sr-only">(current)
          </span></a>
        </li>';
        for ($i = $currentPage + 1; $i < $currentPage + 3; ++$i) {
            if ($i < $pageNumber) {
                $pagination .= '<li class="page-item"><a class="page-link" href="'
                    . $url . '/' . $i . '/?number=' . $numberElement . '">' . $i
                    . '</a></li>';
            }
        }
        if ($currentPage < $pageNumber) {
            ...
            <li class="page-item"><a class="page-link" href="' . $url . '/'
                . $pageNumber . '/?number=' . $numberElement . '">' . $pageNumber
                . '</a></li>
            <li class="page-item">
              <a class="page-link" href="' . $url . '/' . ($currentPage + 1)
                . '/?number=' . $numberElement . '" aria-label="Next">
    ...
    : string
    {
        ...
        foreach ($depts as $departement) {
            ...
            $string .= '<option'. $selected .' value="'
                . $departement->getIdDepartment() . '">' . $departement->getName()
                . '</option>';
        }
        return $string;
    }
     ...
     *
     * Cette méthode crée un lien qui redirige vers une page de modification
     * spécifiée.
     * Le lien est affiché sous la forme d'un élément '<a>' HTML avec le texte
     * "Modifier".
     *
     * @param string $link L'URL vers laquelle le lien doit rediriger pour modifier
     *                     l'élément.
     *
     ...
     *
     * Cette méthode crée un input de type checkbox avec un nom et un identifiant
     * spécifiés.
     * La case à cocher est utilisée pour sélectionner un ou plusieurs éléments dans
     * un formulaire.
     *
     * @param string     $name Le nom de la case à cocher, utilisé pour regrouper
     *                         les cases dans le formulaire.
     * @param string|int $id   L'identifiant unique associé à cette case à cocher,
     *                         utilisé comme valeur lorsqu'elle est sélectionnée.
     *
    ...
    {
        return '<input type="checkbox" name="checkboxStatus' . $name . '[]" value="'
            . $id . '"/>';
    }

    /**
     * Génère le début d'un conteneur de sélection multiple sous forme de navigation
     * par onglets.
     *
     * Cette méthode crée un élément HTML '<nav>' contenant une structure de
     * navigation par onglets pour les sélections multiples. Cela peut être utilisé
     * pour organiser des options ou des catégories, permettant à l'utilisateur de
     * choisir plusieurs éléments de manière intuitive.
     *
    ...
    /**
     * Génère un élément de titre de navigation pour un onglet dans une interface
     * utilisateur.
     *
     * Cette méthode crée un lien HTML qui sert de titre pour un onglet de
     * navigation.
     * Il peut être marqué comme actif en fonction de l'argument fourni.
     *
     * @param string $id     L'identifiant de l'onglet, utilisé pour les attributs
     *                       'id' et 'href'.
     * @param string $title  Le texte affiché pour le titre de l'onglet.
     * @param bool   $active Indique si l'onglet doit être marqué comme actif (true)
     *                       ou non (false). Par défaut, il est false.
     *
    ...
    public function displayTitleSelect($id, $title, $active = false) : string
    {
        ...
        $string .= '" id="nav-' . $id . '-tab" data-toggle="tab" href="#nav-' . $id
     ...
     *
     * Cette méthode crée le code HTML nécessaire pour fermer les éléments de
     * navigation et ouvrir le conteneur de contenu associé aux onglets. Elle est
     * généralement utilisée après l'ajout de tous les titres d'onglets pour
     * compléter la structure de l'interface utilisateur.
     *
     * @return string Le code HTML de la fermeture de la navigation et du début du
     ...
     *
     * @param string $id      L'identifiant unique de l'onglet, utilisé pour le
     *                        lien entre l'onglet et son contenu.
     * @param string $content Le contenu à afficher dans l'onglet.
     * @param bool   $active  Indique si cet onglet doit être actif (visible
     *                        par défaut). Par défaut, il est à false.défaut).
    ...
    public function displayContentSelect($id, $content, $active = false) : string
    {
        ...
        $string .= '" id="nav-' . $id . '" role="tabpanel" aria-labelledby="nav-'
            . $id . '-tab">' . $content . '</div>';
        return $string;
    }
     ...
     *
     * @param string      $title    Le titre du modal, affiché en haut de la
     *                              fenêtre modale.
     * @param string      $content  Le contenu à afficher à l'intérieur
     *                              du modal.
     ...
     *                              le bouton fermera simplement le modal sans
     *                              redirection.
     *
     * @return void Cette méthode n'a pas de valeur de retour, elle affiche
     *              directement le modal à l'utilisateur.
     *
     * @version 1.0
     * @date    2024-10-15
     */
    public function buildModal($title, $content, $redirect = null)
    {
        ...
        if (empty($redirect)) {
            $modal .= '<button type="button" class="btn button_ecran" onclick="$(
    \'#myModal\').hide(); $(\'#modalBackdrop\').hide();">Fermer</button>';
        } else {
            $modal .= '<button type="button" class="btn button_ecran" 
            onclick="document.location.href =\' ' . $redirect . ' \'">Fermer
            </button>';
        }
        $modal .= '</div>
            </div>
    ...
    /**
     * Affiche un modal d'erreur pour indiquer que les mots de passe fournis sont
     * incorrects.
     *
    ...
    {
        $this->buildModal(
            'Mauvais mot de passe', '<p class=\'alert alert-danger\'>
Les deux mots de passe ne sont pas corrects. </p>'
        );
    }

    /**
     * Affiche un modal d'erreur indiquant les problèmes d'enregistrement des
     * utilisateurs.
     *
     * Cette méthode génère un message d'alerte pour chaque utilisateur dont
     * l'enregistrement a échoué en raison d'un problème avec le login ou l'email.
     * Les messages sont affichés dans un modal Bootstrap avec une alerte rouge pour
     * signaler les erreurs.
     *
     * @param array $doubles Un tableau contenant les identifiants des utilisateurs
     *                       qui ont rencontré des problèmes d'enregistrement.
     *
     * @return void Cette méthode n'a pas de valeur de retour, elle affiche
     *              directement le modal d'erreur à l'utilisateur.
     *
     * @version 1.0
     * @date    2024-10-15
     */
    public function displayErrorDouble($doubles)
    {
        ...
        foreach ($doubles as $double) {
            $content .= '<p class="alert alert-danger">' . $double
                . ' a rencontré un problème lors de l\'enregistrement, 
vérifiez son login et son email !</p>';
        }
     ...
     *
     * @return void Cette méthode n'a pas de valeur de retour, elle affiche
     *              directement le modal de confirmation à l'utilisateur.
     *
     * @version 1.0
     * @date    2024-10-15
     */
    ...
    {
        $this->buildModal(
            'Inscription validée', '<p class=\'alert alert-success\'>
Votre inscription a été validée.</p>'
        );
    }
     ...
     * Si une URL de redirection est fournie, l'utilisateur sera redirigé vers cette
     * URL après la confirmation.
     *
     * @param string|null $redirect (optionnel) URL vers laquelle rediriger
     *                              l'utilisateur après la confirmation.
     *
     * @return void
     *
    ...
    {
        $this->buildModal(
            'Modification réussie', '<p class="alert alert-success"> 
La modification a été appliquée.</p>', $redirect
        );
    }
    ...
    {
        $this->buildModal(
            'Erreur lors de l\'inscription', '<p class="alert 
alert-danger"> Le login ou l\'adresse mail est déjà utilisé(e). </p>'
        );
    }


    /**
     * Affiche un modal d'erreur indiquant que le formulaire n'a pas été correctement
     * rempli.
     *
    ...
    {
        $this->buildModal(
            'Le formulaire n\'a pas été correctement remplie', '<p 
class="alert alert-danger">Le formulaire a été mal rempli, veuillez revoir les 
données rentrées et réessayez.</p>'
        );
    }
    /**
     ...
     * Elle informe l'utilisateur de vérifier ses informations avant de soumettre à
     * nouveau.
     *
     * @return void
     *
    ...
    {
        $this->buildModal(
            'L\'ajout a échoué', '<p class="alert alert-danger">Une 
erreur s\'est produite lors de l\'envoi du formulaire, veuillez réessayer après avoir
 vérifié vos informations.</p>'
        );
    }
}
