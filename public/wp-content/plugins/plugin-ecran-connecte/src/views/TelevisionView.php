...
class TelevisionView extends UserView
{
    ...
    public function displayAllTv($users, $userDeptList)
    {
        ...
        foreach ($users as $user) {
            ...
            $row[] = [$count,
                ...
                $user->getLogin(),
                sizeof($user->getCodes()), $userDeptList[$count-1],
     ...
     *
     * @param object $user       L'utilisateur à modifier, qui doit implémenter les
     *                           méthodes nécessaires pour récupérer le login et les
     *                           codes d'emploi du temps.
        ...
        foreach ($user->getCodes() as $code) {
            ...
            if ($count == 1) {
                ...
                $string .= '
                    ...
                    '<input type="button" id="selectId' . $count . '" onclick="
					deleteRow(this.id)" class="btn button_ecran" value="Supprimer">
					</div>';
            }
        }
     ...
     * @param object|null $code       Un objet représentant le code d'emploi du temps
     *                                à pré-sélectionner (facultatif).
     * @param int         $count      Un compteur utilisé pour générer un ID unique
     *                                pour le '<select>' (par défaut à 0).
     *
