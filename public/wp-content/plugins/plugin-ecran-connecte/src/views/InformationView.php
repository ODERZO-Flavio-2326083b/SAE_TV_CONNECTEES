...
class InformationView extends View
{
    ...
    /**
     * TODO : Ajouter la doc pour les paramètres "$allDepts", "$isAdmin" et "$currDept"
    ...
    public function displayFormText(
        array $allDepts, bool $isAdmin = false, int $currDept = null, $title = null,
        ...
        <form method="post">
            <div class="form-group">
                ...
                minlength="4" maxlength="40" placeholder="Titre..." 
                value="' . $title . '">
            </div>
            <div class="form-group">
        ...
        if ($type == 'submit') {
            $form
                .= '<button type="submit" class="btn delete_button_ecran" 
name="delete" onclick="return confirm(
     ...
     * légende. Le champ de date d'expiration est requis et ne peut pas être
...
name="delete" onclick="return confirm(\' Voulez-vous supprimer cette information ?\'
);">Supprimer</button>';
        }
     ...
     * @date    2024-10-15
     */
    public function displayFormVideo($title = null, $content = null, $endDate = null,
        $type = "createVideo"
    ) : string {
        $dateMin = date('Y-m-d', strtotime("+1 day"));
        ...
        $form = '<form method="post" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    placeholder="Insérer un titre" maxlength="60" value="'
            . $title . '">
                </div>';
        if ($content != null) {
            $form .= '
...
Supprimer</button>';
     ...
     * @date    2024-10-15
     */
    public function displayFormShort($title = null, $content = null, $endDate = null,
        $type = "createShort"
    ) : string {
        $dateMin = date('Y-m-d', strtotime("+1 day"));
...
Supprimer</button>';
    ...
    /**
     * Affiche un formulaire pour créer ou modifier un tableau à partir d'un fichier
     * XLS/XLSX avec des champs pour le titre, le fichier à télécharger et la date
     * d'expiration.
     *
     * Le formulaire permet à l'utilisateur d'insérer un titre optionnel et de
     * télécharger un fichier de type Excel. Si un contenu est déjà présent, le
     * tableau correspondant sera affiché. Le champ de date d'expiration est requis
     * et ne peut pas être antérieur à la date actuelle.
     *
     * @param string|null $title   Le titre du tableau à afficher dans le champ
     *                             (optionnel).
     * @param string|null $content Le nom du fichier du tableau à afficher
     *                             (optionnel).
     * @param string|null $endDate La date d'expiration à afficher
     *                             (optionnel).
     * @param string      $type    Le type d'action à effectuer, par
     *                             défaut "createTab". Peut être "submit"
     *                             pour soumettre le formulaire.
     *
     * @return string                 Une chaîne HTML contenant le formulaire.
     *
     ...
     * @date    2024-10-15
     */
    public function displayFormTab($title = null, $content = null, $endDate = null,
        $type = "createTab"
    ) : string {
        $dateMin = date('Y-m-d', strtotime("+1 day"));
        ...
        $form = '<form method="post" enctype="multipart/form-data">
                        <div class="form-group">
                            <label for="title">Titre <span class="text-muted">
                            (Optionnel)</span></label>
                            <input id="title" class="form-control" type="text" 
                            name="title" placeholder="Inserer un titre" 
                            maxlength="60" value="' . $title . '">
                        </div>';
        if ($content != null) {
        ...
        $form .= '
            <div class="form-group">
                ...
                name="contentFile" />
                <input type="hidden" name="MAX_FILE_SIZE" value="5000000" />
                <small id="tabHelp" class="form-text text-muted">Nous vous 
                conseillons de ne pas dépasser trois colonnes.</small>
                <small id="tabHelp" class="form-text text-muted">Nous vous 
                conseillons également de ne pas mettre trop de contenu dans une 
                cellule.</small>
            </div>
            <div class="form-group">
                <label for="expirationDate">Date d\'expiration</label>
...
name="delete" onclick="return confirm(\' Voulez-vous supprimer cette information ?\'
     ...
     * correspondant sera affiché dans un iframe. Le champ de date d'expiration est
     * requis et ne peut pas être antérieur à la date actuelle.
     *
...
name="delete" onclick="return confirm(\' Voulez-vous supprimer cette information ?\'
     ...
     * (images ou PDF) à télécharger. La date d'expiration est requise et ne peut pas
...
name="delete" onclick="return confirm(\' Voulez-vous supprimer cette information ?\'
     ...
     * spécifié (texte, image, vidéo, short, tableau, PDF ou événement).
     *
     ...
     *                        'text', 'img', 'video', 'short' 'tab', 'pdf', 'event').
     *
     * @return string           Une chaîne HTML contenant le lien de retour et le
     *                          formulaire de modification approprié pour le type
     *                          d'information.
     *
     * @version 1.0
     * @date    2024-10-15
    ...
    : string
    {
        if ($type == "text") {
            return '<a href="'
                ...
                . '">< Retour</a>'
                . $this->displayFormText($title, $content, $endDate, 'submit');
        } elseif ($type == "img") {
            return '<a href="'
                . esc_url(
                    get_permalink(
                ...
                . '">< Retour</a>'
                . $this->displayFormImg($title, $content, $endDate, 'submit');
        } elseif ($type == "video") {
            return '<a href="'
                . esc_url(
                    get_permalink(
                ...
                . '">< Retour</a>'
                . $this->displayFormVideo($title, $content, $endDate, 'submit');
        } elseif ($type == "short") {
            return '<a href="'
                . esc_url(
                    get_permalink(
                ...
                . '">< Retour</a>'
                . $this->displayFormShort($title, $content, $endDate, 'submit');
        } elseif ($type == "tab") {
            return '<a href="'
                . esc_url(
                    get_permalink(
                ...
                . '">< Retour</a>'
                . $this->displayFormTab($title, $content, $endDate, 'submit');
        } elseif ($type == "pdf") {
            return '<a href="'
                . esc_url(
                    get_permalink(
                ...
                . '">< Retour</a>'
                . $this->displayFormPDF($title, $content, $endDate, 'submit');
        } elseif ($type == "event") {
     ...
     * disponibles pour les utilisateurs concernant la gestion des informations. Elle
     * inclut des détails sur la façon de visualiser, modifier ou supprimer des
...
class="alert alert-success"> L\'information a été ajoutée. </p>', $linkManageInfo
     ...
     *
     * Cette méthode génère un modal indiquant que l'information a été modifiée avec
     * succès.
     * Elle crée également un lien vers la page de gestion des informations pour
    ...
    public function displayModifyValidate()
    {
        ...
        $linkManageInfo = get_permalink($page->ID);
        $this->buildModal(
            'Modification d\'information validée', '<p class="alert 
alert-success"> L\'information a été modifiée. </p>', $linkManageInfo
        );
    }
     ...
     * peut pas modifier une information, car celle-ci appartient à un autre
     * utilisateur.
