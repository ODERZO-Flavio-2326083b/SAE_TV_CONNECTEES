...
class ICSView extends View
{
     ...
     * fournis dans les données ICS. Elle ajuste également l'affichage en fonction du
     * rôle de l'utilisateur connecté et prend en compte si l'affichage doit se faire
     * pour toute la journée ou non.
     *
            ...
            foreach (array_keys((array)$ics_data['events']) as $year) {
                for ($m = 1; $m <= 12; $m++) {
                    ...
                    if (array_key_exists($month, (array)$ics_data['events'][$year])
                        ...
                        foreach ((array)$ics_data['events'][$year][$month] as $day =>
                            $day_events) {
                            // EN-TÊTE
                            if ($current_study > 9) {
                                break;
                            ...
                            if (in_array(
                                'television', $current_user->roles
                            ) || in_array('technicien', $current_user->roles)
                            ) {
                                if ($day == date('j')) {
     ...
     *
     * Cette méthode retourne un titre HTML ('<h2>') contenant la date complète
     * (jour de la semaine, jour, mois) formatée en fonction de la localisation de
     * WordPress. La méthode ajoute un jour au jour fourni pour calculer le jour de
     * la semaine.
     *
     * @param int $day   Le jour du mois.
     * @param int $month Le mois de
     *                   l'année.
        ...
        if (!(date("H:i", strtotime($event['fin'])) <= $time) || $day != date('j')) {
            ...
            if (in_array("technicien", $current_user->roles)) {
                return $this->displayLineSchedule(
                    [$duration, $event['location']], $active
                );
            } else {
                return $this->displayLineSchedule(
                    [$duration, $label, $description, $event['location']], $active
                );
            }
        }
        return false;
     ...
     * fournies. Si l'événement est actif, elle applique une classe CSS spéciale pour
     ...
     * @param bool  $active Un indicateur optionnel pour déterminer si la ligne doit
                            être marquée comme active (par défaut, 'false').
     *
     ...
     *
     * @param string  $title        Le titre à afficher dans le
     *                              message.
     ...
     *                            cours, ou false si aucun message ne doit être *
     *                            affiché.
     *
     * @version 1.0
     * @date    2024-10-15
