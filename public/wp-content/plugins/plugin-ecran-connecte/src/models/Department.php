...
class Department extends Model implements Entity, JsonSerializable {
     ...
     *
     * Cette méthode prépare et exécute une requête SQL pour récupérer les informations
     * d'un département spécifique à partir de son ID. Elle retourne un objet `Department`
     * si le département est trouvé, ou `false` si aucune donnée n'est trouvée.
     *
     * @param int $id L'ID du département à récupérer.
     *
     * @return bool|Department Un objet `Department` si le département existe, sinon `false`.
     *
     * @version 1.0
     * @date 2024-01-07
     */
     ...
     *
     * Cette méthode prépare et exécute une requête SQL pour récupérer une liste de départements
     * depuis la base de données, avec un nombre limité de résultats en fonction des paramètres
     * de pagination fournis. Elle retourne un tableau d'objets `Department` correspondant aux départements
     * dans la plage spécifiée.
     *
     * @param int $begin Le point de départ (index) pour la récupération des départements (par défaut 0).
     * @param int $numberElement Le nombre de départements à récupérer (par défaut 25).
     *
     * @return array Un tableau d'objets `Department` correspondant aux départements récupérés.
     *
     * @version 1.0
     * @date 2024-01-07
     */
     ...
     *
     * Cette méthode prépare et exécute une requête SQL pour rechercher un département en
     * fonction de son nom dans la base de données. Elle retourne un tableau contenant
     * les résultats correspondant au nom donné.
     *
     * @param string $name Le nom du département à rechercher.
     *
     * @return array|Department Un tableau d'objets `Department` correspondant aux départements trouvés,
     *                           ou un seul objet `Department` si un seul résultat est trouvé.
     *
     * @version 1.0
     * @date 2024-01-07
     */
     ...
     *
     * Cette méthode prend un tableau associatif de données et crée un nouvel objet `Department` en
     * initialisant ses propriétés à partir des valeurs du tableau. Elle retourne ensuite l'entité `Department` ainsi créée.
     *
     * @param array $data Tableau associatif contenant les données du département à utiliser pour l'entité.
     *
     * @return Department L'entité `Department` créée et initialisée avec les données fournies.
     *
     * @version 1.0
     * @date 2024-01-07
     */
     ...
     * @inheritDoc
     *
     * Crée et retourne une liste d'objets `Department` à partir des données fournies.
     *
     * Cette méthode prend un tableau de données et pour chaque entrée dans le tableau, elle crée un objet
     * `Department` en utilisant la méthode `setEntity`, puis ajoute l'objet créé à une liste. La liste complète
     * d'objets `Department` est ensuite retournée.
     *
     * @param array $dataList Tableau contenant les données des départements à utiliser pour créer les entités.
     * @param bool $adminSite (Optionnel) Indicateur de l'état du site administrateur, non utilisé dans cette méthode.
     *
     * @return Department[] Tableau d'objets `Department` créés à partir des données fournies.
     *
     * @version 1.0
     * @date 2024-01-07
     */
     ...
     *
     * Cette méthode interroge la base de données pour récupérer tous les départements enregistrés,
     * puis utilise la méthode `setEntityList` pour convertir les résultats en une liste d'objets `Department`.
     * La liste complète des départements est ensuite retournée.
     *
     * @return Department[] Tableau d'objets `Department` représentant tous les départements stockés dans la base de données.
     *
     * @version 1.0
     * @date 2024-01-07
     */
     ...
     *
     * Cette méthode effectue une jointure entre la table des départements et la table des relations
     * utilisateur-département pour récupérer le département de l'utilisateur spécifié par son ID.
     * Le département correspondant est ensuite renvoyé sous forme d'un objet `Department`.
     *
     * @param int $userId L'ID de l'utilisateur dont on souhaite récupérer le département.
     *
     * @return Department L'objet `Department` représentant le département associé à l'utilisateur.
     *
     * @version 1.0
     * @date 2024-01-07
     */
     ...
     *
     * Cette méthode permet d'obtenir le nom du département associé à l'objet courant.
     *
